...
const SentBubble = ({ message, time }: { message: string, time: string }) => {
    return (
        <div className='max-w-[70%] bg-background dark:bg-card w-fit rounded-xl ml-auto flex'>
            <div className='p-4 bg-background dark:bg-card rounded-lg break-all w-fit'>
                {message}
            </div>
            <p className='text-xs mt-auto mr-3 mb-2 text-nowrap text-gray-700 dark:text-gray-300'>{time}</p>
